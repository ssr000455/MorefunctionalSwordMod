...
public class PinkDiamondGemItem extends Item {
    public PinkDiamondGemItem(Settings settings) {
        super(settings);
    }
    ...
    public void inventoryTick(ItemStack stack, World world, net.minecraft.entity.Entity entity, int slot, boolean selected) {
        if (!world.isClient && entity instanceof PlayerEntity player) {
            if (selected || player.getOffHandStack() == stack) {
                ...
                player.addStatusEffect(new StatusEffectInstance(StatusEffects.SPEED, 220, 0, true, false, true));
            }
        }
    }
}
